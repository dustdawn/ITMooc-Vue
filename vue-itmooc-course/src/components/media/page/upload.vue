...
<script>
  ...
  export default{
    ...
    beforeDestroy() {
      ...
      WebUploader.Uploader.unRegister("webUploaderMine")
    },
    mounted(){
//      var fileMd5;
//      var uploadFile;
      ...
      WebUploader.Uploader.register({
          'name': 'webUploaderMine',
          "before-send-file":"beforeSendFile",
          "before-send":"beforeSend",
          "after-send-file":"afterSendFile"
        },{
          beforeSendFile:function(file) {
            console.log("向服务端请求注册上传文件")
            var that = this;
...
//                alert(this.fileMd5 )
      ...
      this.uploader.on("uploadError", function(file,reason) {
        //console.log("上传错误触发",file,reason)

      });
      //上传成功触发
      this.uploader.on("uploadSuccess", function(file,response ) {
        //console.log("上传成功触发",file,response)
