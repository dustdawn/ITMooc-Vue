  ...
  export default {
    data() {
      return {
        ...
        dialogVisible: false,
        fileList:[],
        uploadval:{courseId : this.courseid},//上传提交的额外的数据 ，将uploadval转成key/value提交给服务器fileTag:"course",businessKey:"testbusiness"
...
//        alert('上传成功')
      ...
      //执行失败 reject
      testPromise(i){
        //调用testPromise方法，执行new Promise中函数，参数为resolve,reject
        return new Promise((resolve,reject)=>{
          if(i<2){
            //成功了
            resolve('成功了');//设置执行成功时(调用then)返回的res
          }else{
            //失败了
            reject('失败了');//设置执行失败catch时返回的res
          }
